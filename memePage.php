			...
			if($result->num_rows > 0) {
				...
				while($row=mysqli_fetch_array($result)){
					...
					?>
						...
						<h3>Likes: <?php echo $row['likes']; ?> <span id="score" title="Score is a measure of how popular you are at the moment. It raises for likes and Tshirt buys, but lowers over time.">Score: </span></h3>
						<img id = "mainMeme" src='memes/<?php echo $row["id"].".png"; ?>' alt='<?php echo $row['title']; ?>' />
					...
					if($resultMemes->num_rows > 0) {
						...
						while($rowMemes=mysqli_fetch_array($resultMemes)){
							echo '<div class="related"><a href = "memePage.php?meme='.$rowMemes["id"].'"><img src="memes/'. $rowMemes["id"].".png".'" alt="'.$rowMemes['title'].'" /></a></div>';
						}
						?>
						</div>
						<?php
